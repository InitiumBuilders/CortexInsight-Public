...
**3.67.** The console leaves the desk. A server has no screen, so `linux/` stands in for the parts of Electron that need one and runs the same main.js: a window made of nothing, a tray that does nothing, and a bus where a renderer would be, which is what lets a terminal show a live feed without a single call site changing. What is genuinely missing is named rather than faked. There is no keyring on a server, so a secret is sealed under a key derived from a 0600 key file, the machine's own id and the user's id, and a vault carried elsewhere is inert even if the key file travels with it. Every channel the app answers goes onto a Unix socket at mode 0600, never a TCP port. `cortex` is that console in a terminal: the reading with the day drawn as a ring, the board, the focus and its drift, the fleet, her commands and protocols, a live feed, and a prompt you can simply talk to. The relay is bundled rather than assumed, configured entirely by environment so it carries no machine's paths and no operator's secrets, and one command installs all of it, asks before anything needing administrator rights, and ends by asking the fleet a question rather than by claiming success. Remote is the other direction: the desktop app reaches a server over SSH, pins its host key on first contact and shows the fingerprint to compare, and requires the vault passphrase on top, because having an account on a machine and being the operator of the console on it are different claims.

The first real Linux boot found three defects that had been true on every platform. Fleet-tree discovery only ran when the setting was empty, and a fresh vault is born holding a placeholder, so it had never run at all: a new machine with a perfectly good tree in its home opened on "no fleet tree is set". The agent queue is append-only with the read position in the vault, so a new vault started at byte zero and replayed a queue another machine had already drained, conjuring 81 tasks and 120 learnings out of finished work. And the vault was written world-readable, which had never mattered on a machine with one person on it and matters immediately on a server. Two more came from the relay: an explicitly empty capability key fell through to a stale key file, and a turn that came back saying the machine was signed out was recorded as OK, which is the worst shape a failure can take because the clean rate stays at 100% while every agent has been mute since the token expired.
...
**3.69.** Greta looked stuck in a loop. She was idle. A relay restart had killed her turn partway through, the runner never got to write that it had finished, and her checkpoint said inflight for half an hour, so the seat looked busy and the hang alarm fired. A turn older than thirty-two minutes whose transcript has been quiet for twelve now reads as interrupted, and the watchdog rewrites its checkpoint, keeps a backup, and says so once. A second cause sat underneath it. A runner marks a turn inflight when it starts, and a second ask that reached the same seat mid-turn read that mark as a crash and resumed the live session, so two processes were running one session at once. The runner now resumes an inflight session only when no process is still running it. Davara's failed run had a different cause. A loop pass at max effort with the full turn budget ran into the relay's thirty-minute cap and lost its work. Passes the app starts by itself now carry their own depth, high effort and at most 48 turns, while the operator's own turns stay at max. The loop prompt names the time limit, and two timeouts in a row pause the loop and say why.

Greta got a second soul and a journal. She scores flow beside craft, meaning, clarity and truth, writes one line on what the work felt like to use, and looks at a live URL herself, at desktop and at phone width, before she judges it. Each evening she writes an entry: the standard to raise tomorrow, what honored the person most, what fell short across seats, where the rhythm broke, and one craft lesson. The standard rides in every seat's brief and in every loop pass. Motus Max reads the notes she is still waiting on before it chooses a move, and a loop whose predictions keep holding comes round sooner.

The Clear archives what the console has moved past: the long bodies of old Duo-Drive receipts, done tasks older than a month, old notifications, and, when asked, fleet logs older than sixty days. It previews first, writes everything it removes into a gzip archive, and checks that the archive reads back before anything goes. The review of 3.68 found twelve defects, and all twelve are fixed. Among them: a Duo-Drive pass could delete the flight marker of the pass that replaced it, the decision ids in the brief could fall outside the 2,000 bytes the bridge hands over (it now hands over 3,600), a spoken gear change swallowed the rest of the sentence, a gear timeout of zero was read as the default, and the reply reader could pick up another seat's file.

*Lesson: a seat that looks stuck may only have missed being told it stopped. Before fixing the loop, check that the record of the last turn is true.*
