...
**The brief.** Written on every change, capped at two thousand characters, prepended to every message by the bridge. Order: the operator's name, the reading with how many open tasks drift from the Motus, Motus, Goal, the open board with short ids, the strongest learnings ranked by endorsement, recurrence and application in closed work. *Accept: the reading is within the first two thousand characters of the brief.*

**The operator's name.** A settings field. One guarded substitution stands where text leaves the app (the relay, the second stack, the agent tool): the founder's name in every prompt becomes the operator's, the month keeps its name, and nothing changes when the operator is the founder. *Accept: "August's board" becomes "<name>'s board" while "born in August 1990" and "the August 5 release" are untouched.*

**The focus as an instrument.** Motus alignment is computed once (word overlap between the Motus and every task) and fed to three actors: a task that drifts for a week is a decision on THE CLOSE, the brief carries the drift count, and the Motus room draws the well (inner orbit aligned, middle near, outer drifting, transform-only rotation paused by the idle brake). An evidence strip counts turns, closes, shipped passes and learnings since a focus was named; every past focus is kept with what it earned; sharpen asks the mind for one line and one falsifier and waits for the operator. The Goal room shows the ladder from the star to today's turns and back up as learning. *Accept: the focus channel carries alignment, evidence and history.*

**Local voice intents.** What the app already knows is answered without a turn: the reading, the board, the arm, the clock, any room by name. Three slow answers in a row turn quick mode on, said out loud. *Accept: "go to the board" resolves locally to the Board.*

**Davara's controls.** Her commands and protocols are pressable: a command types itself on Command, a protocol types its invocation, and her console asks her through one chosen protocol with the reply inline. Nothing spends a turn until the button that says so.
...
**The harnesses.** `--smoke`: a sandbox copy of the vault, every view rendered, every feature contract asserted, screenshots at desktop and 375 px, a report file. `--freshtest`: an empty vault, no fleet tree, the setup gate, every room, a failure on any room that throws, stays blank, is painted invisible, or leaves the operator without a next step; captures taken after real frames. `--fleettest`: a synthetic fleet tree built in a throwaway home (two seats over two days, a proxy log, a checkpoint, memory, a transcript with a file write and usage), a fresh vault pointed at it, then the readers, the frozen index, the checkpoint and its transcript, the file writes, the metered usage, the brief with the reading first, the agent tool on disk, a task through the inbox and a corroborated done claim, and the rooms that show all of it. `--clicktest`: synchronous work behind every start button, cold and warm. An open-source gate that scans every file that ships for secret shapes, private names, tunnel hostnames, baked credentials and personal defaults, proven on planted decoys. A continuous build that runs the gate, the fresh harness and the packager on Windows and macOS runners on every push, and turns a version tag into a Release.
